...
class FastAPIMCPOpenAPI:
    ...
    def _mount_mcp_server(self):
                    ...
                    if transport_type:
                        ...
                        response = Response(
                            content=json.dumps({
                                ...
                                "tools": ["list_endpoints", "get_endpoint_docs"]
                            }),
                            media_type="application/json",
                            headers={
                                "Access-Control-Allow-Origin": "*",
                                    ...
                                    if tool_name == "list_endpoints":
                                        ...
                                        for route in self.app.routes:
                                            if isinstance(route, APIRoute):
                                                ...
                                                endpoint_info = {
                                                    ...
                                                    "methods": list(route.methods),
                                                    "name": route.name,
                                                    "summary": getattr(route.endpoint, "__doc__", "").split("\n")[0] if route.endpoint and getattr(route.endpoint, "__doc__", None) else None
                                                }
                                                endpoints.append(endpoint_info)
                                        
                                        result_content = json.dumps(endpoints, indent=2)
                                        
                                    elif tool_name == "get_endpoint_docs":
                                        ...
                                        if "paths" in openapi_schema and endpoint_path in openapi_schema["paths"]:
                                            ...
                                            if method.lower() in path_item:
                                                ...
                                                result_content = json.dumps(endpoint_schema, indent=2)
                                            else:
        ...
        @self.app.post(self.mount_path)
        @self.app.get(self.mount_path) 
        @self.app.options(self.mount_path)
