...
def test_library_functionality():
    ...
    try:
        ...
        raise AssertionError(f"Library initialization failed: {e}") from e
    ...
    try:
        ...
        raise AssertionError(f"Endpoint introspection failed: {e}") from e
        ...
        if "paths" in openapi_schema:
            ...
            if "/" in openapi_schema["paths"]:
                ...
                if "get" in root_endpoint:
                    ...
                    raise AssertionError("Root endpoint GET method not found")
            else:
                print("❌ Root endpoint not found in OpenAPI schema")
                raise AssertionError("Root endpoint not found in OpenAPI schema")
        else:
            print("❌ No paths found in OpenAPI schema")
            raise AssertionError("No paths found in OpenAPI schema")

    except Exception as e:
        print(f"❌ Error during OpenAPI schema generation: {e}")
        raise AssertionError(f"OpenAPI schema generation failed: {e}") from e
